...
class Better_Post_Views {
	...
	/**
	 * return daily meta key ID
	 *
	 * @return string
	 */
	public static function get_meta_id_daily() {

		return self::get_option( 'meta_id_daily' );
	}


	/**
	 * return 7 days meta key ID
	 *
	 * @return string
	 */
	public static function get_meta_id_7days_data() {

		return self::get_option( 'meta_id_7days_data' );
	}


	/**
	 * return 7 days total meta key ID
	 *
	 * @return string
	 */
	public static function get_meta_id_7days_total() {

		return self::get_option( 'meta_id_7days_total' );
	}
	...
	function increment_views( $post_id ) {
		...
		if ( ! $current_day_views = get_post_meta( $post->ID, self::get_meta_id_daily(), true ) ) {
			$current_day_views = 1;
		} else {
			$current_day_views += 1;
		}

		update_post_meta( $post->ID, self::get_meta_id_daily(), $current_day_views );
		...
		$last_7day_data = get_post_meta( $post->ID, self::get_meta_id_7days_data(), true );
		...
		update_post_meta( $post->ID, self::get_meta_id_7days_data(), $last_7day_data );

		// Update last 7 days total views
		update_post_meta( $post->ID, self::get_meta_id_7days_total(), $last_7day_sum );
	...
	public static function get_views( $post_id = 0, $prefix = '', $postfix = '', $template = '' ) {
		...
		// Get post view count
		if ( ! $post_views = get_post_meta( $post_id, self::get_meta_id_daily(), true ) ) {
			$post_views = 0;
		}
	...
	function handle_admin_sortable_columns( $query ) {
		...
		if ( 'better-post-views' == $orderby ) {
			$query->set( 'meta_key', self::get_meta_id_daily() );
