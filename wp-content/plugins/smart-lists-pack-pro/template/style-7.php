<?php
/**
 * The template for smart list style 7
 *
 * @var $smart_list
 *
 * @author     BetterStudio
...
{
	$ad_data = bssl_get_ad_location_data( 'bssl_style_7', FALSE, array(
		'each' => 5,
	) );
...
if ( isset( $smart_list['before'] ) ) {
	echo '<span class="bs-smart-list-start bssl-before-style-7"></span>';
}

?>
	<div class="bs-smart-list bssl-style-7 bssl-t1 bssl-s7">
				...
				foreach ( $smart_list['items'] as $item ) {
					...
					if ( $ad_data['active_location'] ) {
						...
						if ( ! ( $ad_counter >= $items_count ) && $ad_counter % $ad_data['each'] === 0 ) {
							bssl_show_ad_location( 'bssl_style_7', array(
								'ad_data' => $ad_data,
							) );
						}
					}
