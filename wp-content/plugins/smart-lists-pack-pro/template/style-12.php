<?php
/**
 * The template for smart list style 12
 *
 * @var $smart_list
 *
 * @author     BetterStudio
...
{
	$ad_data = bssl_get_ad_location_data( 'bssl_style_12', FALSE, array(
		'each' => 5,
	) );
...
if ( isset( $smart_list['before'] ) ) {
	echo '<span class="bs-smart-list-start bssl-before-style-12"></span>';
}

?>
	<div class="bs-smart-list bssl-style-12 bssl-t1 bssl-s12">
				...
				foreach ( $smart_list['items'] as $item ) {
					...
					<div class="bssl-item bs-slider-item">
						...
						if ( ! empty( $src ) ) {
							...
							?>
							<figure class="bssl-image-w bssl-count-type-badge">
						...
						?>
						<h2 class="bssl-item-title heading-typo <?php echo $src ? '' : ' bssl-count-type-badge' ?>">
							<?php if ( ! $src ) { ?>
								<span class="bssl-count"><?php echo number_format_i18n( $counter ); ?></span>
							<?php } ?>
							<?php echo $item['title']; ?>
					...
					if ( $ad_data['active_location'] ) {
						...
						if ( ! ( $ad_counter >= $items_count ) && $ad_counter % $ad_data['each'] === 0 ) {
							bssl_show_ad_location( 'bssl_style_12', array(
								'ad_data' => $ad_data,
							) );
						}
					}
