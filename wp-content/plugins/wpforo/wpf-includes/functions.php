...
function wpforo_db_check( $args = array() ){
	...
	switch( trim( wpfval($args, 'check') ) ){
        ...
        case 'col_exists':
	        return (bool) $wpdb->get_var("SHOW COLUMNS FROM `{$table}` LIKE '{$col}'");
        case 'key_exists':
	        return (bool) $wpdb->get_var("SHOW KEYS FROM `{$table}` WHERE `Key_name` = '{$col}'");
        case 'default_value':
	        $c = (array) $wpdb->get_row("SHOW COLUMNS FROM `{$table}` LIKE '{$col}'", ARRAY_A);
	        return wpfval($c, 'Default');
        case 'col_type':
	        $c = (array) $wpdb->get_row("SHOW COLUMNS FROM `{$table}` LIKE '{$col}'", ARRAY_A);
	        return wpfval($c, 'Type');
	}
