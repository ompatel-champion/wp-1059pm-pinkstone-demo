<?php
/**
 * The template for smart list style 11
 *
 * @var $smart_list
 *
 * @author     BetterStudio
...
{
	$ad_data = bssl_get_ad_location_data( 'bssl_style_11', FALSE, array(
		'each' => 5,
	) );
...
?>
	<div class="bs-smart-list bssl-style-11 bssl-t1 bssl-s11">
		<div class="bssl-inner">

			<div class="bssl-items bssl-before-style-11">
				<?php
				...
				foreach ( $smart_list['items'] as $item ) {
					...
					if ( $ad_data['active_location'] ) {
						...
						if ( ! ( $ad_counter >= $items_count ) && $ad_counter % $ad_data['each'] === 0 ) {
							bssl_show_ad_location( 'bssl_style_11', array(
								'ad_data' => $ad_data,
							) );
						}
					}
